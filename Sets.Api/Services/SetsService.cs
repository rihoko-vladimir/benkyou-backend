...
public class SetsService : ISetsService
{
    ...
    public async Task<Result<PagedSetsResponse>> GetAllSetsAsync(Guid userId, int pageNumber, int pageSize,
        string searchQuery)
    {
    ...
    public async Task<Result> FinishSetLearning(Guid userId, Guid setId, FinishLearningRequest finishLearningRequest)
    {
        Log.Information("User {UserId} finished learning set {SetId}", userId, setId);

        var setResult = await GetSetAsync(setId);

        if (!setResult.IsSuccess) return Result.Error("No such set is present");

        var set = setResult.Value!;

        var chars = set.KanjiList.Select(kanji => kanji.KanjiChar[0]).OrderBy(c => c).ToList();

        var resultChars = finishLearningRequest.KanjiResults.Select(result => result.Kanji).OrderBy(c => c).ToList();

        if (!chars.SequenceEqual(resultChars)) return Result.Error("Incorrect result provided");

        return await _senderService.SendLearnFinishResultMessage(
            userId,
            setId,
            finishLearningRequest.StartTime,
            finishLearningRequest.EndTime,
            finishLearningRequest.KanjiResults
        );
    }
}