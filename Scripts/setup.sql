IF NOT EXISTS (SELECT * FROM sys.databases WHERE name = 'Benkyou_auth')
BEGIN
  CREATE DATABASE Benkyou_auth
END;
GO

IF NOT EXISTS (SELECT * FROM sys.databases WHERE name = 'Benkyou_sets')
BEGIN
  CREATE DATABASE Benkyou_sets
END;
GO

IF NOT EXISTS(SELECT *
              FROM sys.databases
              WHERE name = 'Benkyou_users')
BEGIN
...
create unique index UsersInformation_Id_uindex
    ...
    go
exec ('create procedure createUser(
...
go
exec ('create procedure getUserById(@user_id AS uniqueidentifier)
            as
            begin
                SELECT *
...
go
exec ('create procedure updateUserAvatar(
                        @user_id as uniqueidentifier,
